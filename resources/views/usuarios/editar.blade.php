...
@section('conteudo')
    ...
    <form action="{{ route('usuarios/editar', $user->id) }}" method="POST" id="formulario">
        @csrf
        @method('put')
        <label for="nome" title="Este campo é obrigatório!">Nome<span class="obrigatorio"> *</span></label>
        <input type="text" name="nome" id="nome" placeholder="Nome do Usuário" value="{{ old('nome', $user->nome ?? '') }}">
        ...
        <select name="tipoUsuario" id="tipoUsuario">
            ...
            @foreach($tiposUsuario as $tipoUsuario)
                @php
                    $selected = ($tipoUsuario == $user->tipoUsuario) ? "selected" : "";
                @endphp
        ...
        <input type="text" name="login" id="login" placeholder="Login do Usuário" value="{{ old('login', $user->login ?? '') }}">
