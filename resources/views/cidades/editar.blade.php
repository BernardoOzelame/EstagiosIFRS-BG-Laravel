...
@section('conteudo')
    ...
    <form action="{{ route('cidades/editar', $cidade->id) }}" method="POST" id="formulario">
        @csrf
        @method('put')
        <label for="nome" title="Este campo é obrigatório!">Nome<span class="obrigatorio"> *</span></label>
        <input type="text" name="nome" id="nome" placeholder="Nome" value="{{ old('nome', $cidade->nome ?? '') }}">
        
        <label for="uf" title="Este campo é obrigatório!">UF<span class="obrigatorio"> *</span></label>
        <input type="text" name="uf" id="uf" placeholder="UF" value="{{ old('uf', $cidade->uf ?? '') }}">
