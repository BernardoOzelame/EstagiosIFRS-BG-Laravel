...
@section('conteudo')
    ...
    <form action="{{ route('empresas/gravar') }}" method="POST" id="formulario">
        @csrf
        ...
        <input type="text" name="cnpj" id="cnpj" placeholder="CNPJ" value="{{ old('cnpj') }}">

        <label for="numConvenio" title="Este campo é obrigatório!">Número do convênio<span class="obrigatorio"> *</span></label>
        <input type="text" name="numConvenio" id="numConvenio" placeholder="Número do convênio" value="{{ old('numConvenio') }}">
        ...
        <select name="cidades_id" id="cidades_id">
            ...
            @foreach($cidades as $cidade)
                <option value="{{ $cidade->id }}" 
                    {{ old('cidades_id') == $cidade->id ? 'selected' : '' }}>
                    {{ $cidade->nome }}
                </option>
            @endforeach
        </select>
