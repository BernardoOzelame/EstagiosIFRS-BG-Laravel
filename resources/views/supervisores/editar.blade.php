...
@section('conteudo')
    ...
    <form action="{{ route('supervisores/editar', $supervisor->id) }}" method="POST" id="formulario">
        @csrf
        @method('put')
        <label for="nome" title="Este campo é obrigatório!">Nome<span class="obrigatorio"> *</span></label>
        <input type="text" name="nome" id="nome" placeholder="Nome" value="{{ old('nome', $supervisor->nome ?? '') }}">
        
        <label for="email" title="Este campo é obrigatório!">E-mail<span class="obrigatorio"> *</span></label>
        <input type="email" name="email" id="email" placeholder="E-mail" value="{{ old('email', $supervisor->email ?? '') }}">

        <label for="cargo" title="Este campo é obrigatório!">Cargo<span class="obrigatorio"> *</span></label>
        <input type="text" name="cargo" id="cargo" placeholder="Cargo" value="{{ old('cargo', $supervisor->cargo ?? '') }}">
        
        <label for="formacao" title="Este campo é obrigatório!">Formação<span class="obrigatorio"> *</span></label>
        <input type="text" name="formacao" id="formacao" placeholder="Formação" value="{{ old('formacao', $supervisor->formacao ?? '') }}">

        <label for="telefoneCelular" title="Este campo é obrigatório!">Celular<span class="obrigatorio"> *</span></label>
        <input type="text" name="telefoneCelular" id="telefoneCelular" placeholder="Celular" value="{{ old('telefoneCelular', $supervisor->telefoneCelular ?? '') }}">
        ...
        <select name="empresas_id" id="empresas_id">
            ...
            @foreach($empresas as $empresa)
                <option value="{{ $empresa->id }}"
                    {{ old('empresas_id', $supervisor->empresas_id ?? '') == $empresa->id ? 'selected' : '' }}>
                    {{ $empresa->nome }}
                </option>
            @endforeach
        </select>
